...
*/
import "C"
import "github.com/sampgo/sampgo/player"
...
func onPlayerConnect(playerid C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)})
}
...
func onPlayerDisconnect(playerid C.int, reason C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(reason))
}
...
func onPlayerDeath(playerid C.int, killerid C.int, reason C.int) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, player.Player, int) bool)
	if !ok {
		return false
	}
	return fn(player.Player{ID: int(playerid)}, player.Player{ID: int(killerid)}, int(reason))
}
...
func onVehicleDeath(vehicleid C.int, killerid C.int) bool {
	...
	fn, ok := evt.Handler.(func(int, player.Player) bool)
	if !ok {
		return false
	}
	fn(int(vehicleid), player.Player{ID: int(killerid)})
	return true
}
...
func onPlayerText(playerid C.int, text *C.char_t) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, string) bool)
	if !ok {
		return true
	}
	return fn(player.Player{ID: int(playerid)}, C.GoString(C.constToNonConst(text)))
}
...
func onPlayerRequestClass(playerid C.int, classid C.int) bool {
	...
	fn(player.Player{ID: int(playerid)}, int(classid))
	return true
}
...
func onPlayerEnterVehicle(playerid C.int, vehicleid C.int, ispassenger C.bool) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, int, bool) bool)
	if !ok {
		return false
	}
	fn(player.Player{ID: int(playerid)}, int(vehicleid), bool(ispassenger))
	return true
}
...
func onPlayerExitVehicle(playerid C.int, vehicleid C.int) bool {
	...
	fn(player.Player{ID: int(playerid)}, int(vehicleid))
	return true
}
...
func onPlayerStateChange(playerid C.int, newstate C.int, oldstate C.int) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, int, int) bool)
	if !ok {
		return false
	}
	fn(player.Player{ID: int(playerid)}, int(newstate), int(oldstate))
	return true
}
...
func onPlayerEnterCheckpoint(playerid C.int) bool {
	...
	fn(player.Player{ID: int(playerid)})
	return true
}
...
func onPlayerObjectMoved(playerid C.int, objectid C.int) bool {
	...
	fn(player.Player{ID: int(playerid)}, int(objectid))
	return true
}
...
func onPlayerPickUpPickup(playerid C.int, pickupid C.int) bool {
	...
	fn(player.Player{ID: int(playerid)}, int(pickupid))
	return true
}
...
func onVehicleMod(playerid C.int, vehicleid C.int, componentid C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(vehicleid), int(componentid))
}
...
func onEnterExitModShop(playerid C.int, enterexit C.bool, interiorid C.int) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, bool, int) bool)
	if !ok {
		return false
	}
	fn(player.Player{ID: int(playerid)}, bool(enterexit), int(interiorid))
	return true
}
...
func onVehiclePaintjob(playerid C.int, vehicleid C.int, paintjobid C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(vehicleid), int(paintjobid))
}
...
func onVehicleRespray(playerid C.int, vehicleid C.int, color1 C.int, color2 C.int) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, int, int, int) bool)
	if !ok {
		return true
	}
	return fn(player.Player{ID: int(playerid)}, int(vehicleid), int(color1), int(color2))
}
...
func onVehicleDamageStatusUpdate(vehicleid C.int, playerid C.int) bool {
	...
	return fn(int(vehicleid), player.Player{ID: int(playerid)})
}
...
func onUnoccupiedVehicleUpdate(vehicleid C.int, playerid C.int, passenger_seat C.int, new_x C.float, new_y C.float, new_z C.float, vel_x C.float, vel_y C.float, vel_z C.float) bool {
	...
	fn, ok := evt.Handler.(func(int, player.Player, int, float32, float32, float32, float32, float32, float32) bool)
	if !ok {
		return true
	}
	return fn(int(vehicleid), player.Player{ID: int(playerid)}, int(passenger_seat), float32(new_x), float32(new_y), float32(new_z), float32(vel_x), float32(vel_y), float32(vel_z))
}
...
func onPlayerSelectedMenuRow(playerid C.int, row C.int) bool {
	...
	fn(player.Player{ID: int(playerid)}, int(row))
	return true
}
...
func onPlayerInteriorChange(playerid C.int, newinteriorid C.int, oldinteriorid C.int) bool {
	...
	fn(player.Player{ID: int(playerid)}, int(newinteriorid), int(oldinteriorid))
	return true
}

//export onPlayerKeyStateChange
func onPlayerKeyStateChange(playerid C.int, newkeys C.int, oldkeys C.int) {
	...
	fn(player.Player{ID: int(playerid)}, int(newkeys), int(oldkeys))
	return
}
...
func onPlayerStreamIn(playerid C.int, forplayerid C.int) bool {
	...
	fn(player.Player{ID: int(playerid)}, int(forplayerid))
	return true
}
...
func onDialogResponse(playerid C.int, dialogid C.int, response C.int, listitem C.int, inputtext *C.char_t) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, int, int, int, string) bool)
	if !ok {
		return false
	}
	return fn(player.Player{ID: int(playerid)}, int(dialogid), int(response), int(listitem), C.GoString(C.constToNonConst(inputtext)))
}
...
func onPlayerTakeDamage(playerid C.int, issuerid C.int, amount C.float, weaponid C.int, bodypart C.int) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, player.Player, float32, int, int) bool)
	if !ok {
		return false
	}
	return fn(player.Player{ID: int(playerid)}, player.Player{ID: int(issuerid)}, float32(amount), int(weaponid), int(bodypart))
}
...
func onPlayerGiveDamage(playerid C.int, damagedid C.int, amount C.float, weaponid C.int, bodypart C.int) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, int, float32, int, int) bool)
	if !ok {
		return false
	}
	return fn(player.Player{ID: int(playerid)}, int(damagedid), float32(amount), int(weaponid), int(bodypart))
}
...
func onPlayerGiveDamageActor(playerid C.int, damaged_actorid C.int, amount C.float, weaponid C.int, bodypart C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(damaged_actorid), float32(amount), int(weaponid), int(bodypart))
}
...
func onPlayerClickMap(playerid C.int, fX C.float, fY C.float, fZ C.float) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, float32, float32, float32) bool)
	if !ok {
		return false
	}
	return fn(player.Player{ID: int(playerid)}, float32(fX), float32(fY), float32(fZ))
}
...
func onPlayerClickTextDraw(playerid C.int, clickedid C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(clickedid))
}
...
func onPlayerClickPlayerTextDraw(playerid C.int, playertextid C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(playertextid))
}
...
func onIncomingConnection(playerid C.int, ip_address *C.char_t, port C.int) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, string, int) bool)
	if !ok {
		return false
	}
	return fn(player.Player{ID: int(playerid)}, C.GoString(C.constToNonConst(ip_address)), int(port))
}
...
func onTrailerUpdate(playerid C.int, vehicleid C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(vehicleid))
}
...
func onVehicleSirenStateChange(playerid C.int, vehicleid C.int, newstate C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(vehicleid), int(newstate))
}
...
func onPlayerClickPlayer(playerid C.int, clickedplayerid C.int, source C.int) bool {
	...
	fn(player.Player{ID: int(playerid)}, int(clickedplayerid), int(source))
	return false
}
...
func onPlayerEditObject(playerid C.int, playerobject C.bool, objectid C.int, response C.int, fX C.float, fY C.float, fZ C.float, fRotX C.float, fRotY C.float, fRotZ C.float) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, bool, int, int, float32, float32, float32, float32, float32, float32) bool)
	if !ok {
		return false
	}
	return fn(player.Player{ID: int(playerid)}, bool(playerobject), int(objectid), int(response), float32(fX), float32(fY), float32(fZ), float32(fRotX), float32(fRotY), float32(fRotZ))
}
...
func onPlayerEditAttachedObject(playerid C.int, response C.int, index C.int, modelid C.int, boneid C.int, fOffsetX C.float, fOffsetY C.float, fOffsetZ C.float, fRotX C.float, fRotY C.float, fRotZ C.float, fScaleX C.float, fScaleY C.float, fScaleZ C.float) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, int, int, int, int, float32, float32, float32, float32, float32, float32, float32, float32, float32) bool)
	if !ok {
		return false
	}
	return fn(player.Player{ID: int(playerid)}, int(response), int(index), int(modelid), int(boneid), float32(fOffsetX), float32(fOffsetY), float32(fOffsetZ), float32(fRotX), float32(fRotY), float32(fRotZ), float32(fScaleX), float32(fScaleY), float32(fScaleZ))
}
...
func onPlayerSelectObject(playerid C.int, type_ C.int, objectid C.int, modelid C.int, fX C.float, fY C.float, fZ C.float) bool {
	...
	fn, ok := evt.Handler.(func(player.Player, int, int, int, float32, float32, float32) bool)
	if !ok {
		return false
	}
	return fn(player.Player{ID: int(playerid)}, int(type_), int(objectid), int(modelid), float32(fX), float32(fY), float32(fZ))
}
...
func onPlayerWeaponShot(playerid C.int, weaponid C.int, hittype C.int, hitid C.int, fX C.float, fY C.float, fZ C.float) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(weaponid), int(hittype), int(hitid), float32(fX), float32(fY), float32(fZ))
}
...
func onPlayerRequestDownload(playerid C.int, type_ C.int, crc C.int) bool {
	...
	return fn(player.Player{ID: int(playerid)}, int(type_), int(crc))
}
